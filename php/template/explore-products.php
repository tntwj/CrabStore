<section class="container mt-5">
    ...
    <?php
        ...
        for ($i = 0; $i < $productCount; $i++) { 
            ...
            echo '<div class="card">';
            echo '<img src="./upload/products/'.$media[0]["imageUrl"].'" class="card-img-top w-50 h-auto" alt="'.$templateParams["product-category"].'">';
            echo '<div class="card-body">
                        <h5 class="card-title">'.$products[$i]['name'].'</h5>
                        <p class="card-text">'.$products[$i]['shortDescription'].'</p>
                        <a href="view-product.php?product='.$products[$i]['productId'].'&category='.$templateParams["product-category"].'&media='.$media[0]["imageUrl"].'" class="btn btn-primary">View Product</a>
                    </div></div></div>';
